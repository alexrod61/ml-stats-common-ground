...
Finally, following the structure of the [MDS Stat-ML dictionary](https://ubc-mds.github.io/resources_pages/terminology/), the @sec-dictionary contains a dictionary that follows the same format which contrasts the terminology a Data Scientist can find in Statistics and Machine Learning when performing supervised learning in practice. Readers might be surprised that certain concepts refer to the same idea but with different terminology. Contrarily, in some other cases, the same terminology might refer to different concepts! 
